...
<table class="topbar-main"><tr>
  ...
  <td class="col-center"><span class="heading">Search</span></td>
...
<div class="page-narrow">
  ...
  <h1>title or author</h1>
  <form class="search-form" method="get" action="/search">
    <input type="text" name="q" value="${escapeHtml(query)}" placeholder="type here…" autofocus>
    <br>
...
function renderPager(query: string, page: number, totalPages: number): string {
  ...
  const prev = page > 1
    ? `<a class="pager-btn" href="${mk(page - 1)}" aria-label="Previous page">${CHEVRON_LEFT}</a>`
    : `<span class="pager-btn disabled" aria-label="First page">${CHEVRON_LEFT}</span>`;
  const next = page < totalPages
    ? `<a class="pager-btn" href="${mk(page + 1)}" aria-label="Next page">${CHEVRON_RIGHT}</a>`
...
<table class="pager" style="margin-top:14px"><tr>
  ...
  <td class="col-center"><strong>${page}</strong>&nbsp;of&nbsp;<strong>${totalPages}</strong></td>
